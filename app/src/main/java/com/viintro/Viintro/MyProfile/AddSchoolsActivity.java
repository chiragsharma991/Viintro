...
public class AddSchoolsActivity extends Activity {
    private EditText edt_schools;
    ...
    {
        edt_schools = (EditText) findViewById(R.id.edt_schools);
        text_char_left = (TextView) findViewById(R.id.text_char_left);
        img_close = (ImageView) findViewById(R.id.img_close);
        btn_saveInfo = (Button) findViewById(R.id.btn_saveInfo);

        String sch = sharedpreferences.getString("sch", null);
        if(!(sch == null)){
            edt_schools.setText(sch);
        }

        edt_schools.addTextChangedListener(new TextWatcher() {
            @Override
    ...
    //API call to send schools data
    public void req_add_schools_API(final Context cont)
    {
        ...
        String url = m_config.web_api_link+ "/add-schools";
        ...
        try
        {
            ...
            obj.put("schools", edt_schools.getText().toString().replaceAll("\\s+", " ").trim());  //m_config.profile_website
        ...
        JsonObjectRequest getRequest = new JsonObjectRequest(Request.Method.POST, url, obj,
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        CommonFunctions.hDialog();
                        CommonFunctions.displayToast(context,"Schools details has been saved");

                        String schools = edt_schools.getText().toString().replaceAll("\\s+", " ").trim();
                        profile_schools_list = new ArrayList();
                        profile_schools_list.add(Arrays.asList(schools.split(",")));
                        Intent resultIntent = new Intent();
                        resultIntent.putExtra("schools", schools);
                            ...
                            if (error.networkResponse.statusCode == 500) {
                                ...
                                if (networkresponse_data != null) {
                                    ...
                                    try {
                                        ...
                                        CommonFunctions.displayToast(context, context.getResources().getString(R.string.Schools_details_failed));
                                    }
        ...
        ViintroApplication.getInstance().addToRequestQueue(getRequest, "add_schools_api");
