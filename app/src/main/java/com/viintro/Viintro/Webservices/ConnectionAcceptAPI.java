...
public class ConnectionAcceptAPI {

    static Context context;
    static Configuration_Parameter m_config;

    //api call for connection accept
    public static void req_connection_accept_API(final Context cont, final int user_id, final int position, final String from_Activity) {
        context = cont;
        ...
        String url = m_config.web_api_link_header+ "/connection/accept";
        ...
        // prepare the Request
        JsonObjectRequest connection_Request = new JsonObjectRequest(Request.Method.POST, url, obj,
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            ...
                            {
                                update_invitation_row_on_suggestion_page(position, user_id, "connect");
                            }
                            else if(from_Activity.equals("Invitations"))
                            {
                                update_row_on_invitation_page(position, user_id, "connect");
                            }
                            else if(from_Activity.equals("Public_Profile"))
                            {
                                upd_btn_on_pub_prof_after_inv_acc_rej(position, user_id, "connect");
                            }
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        if(error.networkResponse != null)
                        {
                            ...
                            if(networkresponse_data != null)
                            {
                                ...
                                {
                                    CommonFunctions.displayToast(context,context.getResources().getString(R.string.Connection_accept_failed));
                                }
        ...
        connection_Request.setRetryPolicy(policy);
        //queue.add(getRequest);

        // Adding request to request queue
        ViintroApplication.getInstance().addToRequestQueue(connection_Request, "connection_accept_api");
