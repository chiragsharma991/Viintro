...
#include "ffmpeg.h"
#include "ffmpeg_opt.c"//.c.h
#include "cmdutils.c"//.c.h

#include "libavutil/avassert.h"
static JNIEnv *env;
static jmethodID messageMe;
static jobject pObj;
...
//            LOGI("frame %d", buf);
			...
			(*env)->CallVoidMethod(env, pObj, messageMe, jstr);
...
int isDecoderAva(char* filename){
	av_register_all();
	const AVCodecDescriptor **codecs;
	unsigned i,j, nb_codecs = get_codecs_sorted(&codecs);
	AVFormatContext* pFormatCtx = avformat_alloc_context();
	avformat_open_input(&pFormatCtx, filename, NULL, NULL);
	avformat_find_stream_info(pFormatCtx,NULL);
	int all_matched=1;
	for(j=0; j<pFormatCtx->nb_streams; j++){
		int stream_matched=0;
		for (i = 0; i < nb_codecs; i++) {
			const AVCodecDescriptor *desc = codecs[i];
			const AVCodec *codec = NULL;
			while ((codec = next_codec_for_id(desc->id, codec,0))) {
				if((desc->type==pFormatCtx->streams[j]->codec->codec_type) && (desc->id==pFormatCtx->streams[j]->codec->codec_id)){
					stream_matched = 1;
				}
			}
		}
		all_matched&=stream_matched;
	}
	av_free(codecs);
	avformat_free_context(pFormatCtx);
	return all_matched;
...
int main(int argc, char **argv,JNIEnv *en, jobject pOb)
{
	env = en;
	pObj = pOb;
	jclass clazz = (*env)->FindClass(env, "com/lkland/videocompressor/nativeadapter/FFmpegAdapter");
	messageMe = (*env)->GetMethodID(env, clazz, "messageme", "(Ljava/lang/String;)V");
	jstring jstr = (*env)->NewStringUTF(env, "test");
//	(*env)->CallVoidMethod(env, pObj, messageMe, jstr);
